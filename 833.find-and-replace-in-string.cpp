...
public:
    string findReplaceString(string S, vector<int>& indexes, vector<string>& sources, vector<string>& targets) {
        ...
        for (int i = 0; i < indexes.size(); i++) {
            index_map[indexes[i]] = i;
        }
        for (int i = 0; i < S.size(); ) {
            if (index_map.find(i) != index_map.end()) {
                replace_idx = index_map[i];
                string &to_match = sources[replace_idx];
                replace_idx++;
                int len = to_match.size();
                if (S.size() - i >= len && S.substr(i, len) == to_match) {
                    ret += targets[replace_idx-1];
                    i += len;
                    continue;
                }
            }
