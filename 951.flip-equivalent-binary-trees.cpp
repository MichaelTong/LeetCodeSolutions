...
/**
 * DFS four conditions
 * 1) both node are nullptr
 * Or both are not nullptr and their value are equal and:
 *      2) their left substree and right substree are flipEquiv respectively
 *      3) or left and right are "crossing" flipEquiv
 */
class Solution {
public:

    bool flipEquiv(TreeNode* t1, TreeNode* t2) {
        return (t1 == nullptr && t2 == nullptr) || 
            t1 != nullptr && t2 != nullptr && t1->val == t2->val &&
            ((flipEquiv(t1->left, t2->left) && (flipEquiv(t1->right, t2->right))) ||
