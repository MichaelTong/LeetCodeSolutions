...
*/
/**
 * The importance of an employee is the sum of all the employees it manages.
 * 
 * This is again a tree structure, basically,we want to get the sum of all
 * subtree nodes.
 */
class Solution {
...
public:
    ...
    int dfs(int id) {
        Employee* this_e = emap[id];
        int imp = this_e->importance;
        for (int subid : this_e->subordinates) {
            imp += dfs(subid);
        }
        return imp;
    }
