...
// @lc code=start
/**
 * Different from getting the delay in the network
 * Since this is a tree structure, there is no need to apply dijkstra's
 * algorithm. Do a BFS to get the latencies.
 * 
 */
class Solution {
public:
    int numOfMinutes(int n, int headID, vector<int>& manager, vector<int>& informTime) {
        ...
        for (int i = 0; i < manager.size(); i++) {
            int mgr = manager[i];
            if (mgr == -1) continue;
            subs[mgr].push_back(i);
        }
        
        vector<int> informed(n, 0);
        int visitied = 0;
        queue<int> to_visit;
        to_visit.push(headID);
        int max_time = 0;
        ...
        while (!to_visit.empty()) {
            ...
            auto &sub = subs[cur];
            for (auto s :sub) {
                informed[s] = informed[cur] + cost;
                max_time = max(max_time, informed[s]);
                to_visit.push(s);
            }
        }
        return max_time;
